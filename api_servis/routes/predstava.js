...
route.get("/:id", async (req, res) => {
  try {
    return res.json("predstave čiji je id=" + req.params.id);
  } catch (err) {
    console.log(err);
    res.status(500).json({ error: "Greska pri citanju", data: err });
  }
...
route.post("/", async (req, res) => {
  try {
    return res.json("unos nove predstave čiji su podaci u req.body");
  } catch (err) {
    console.log(err);
...
route.put("/:id", async (req, res) => {
  try {
    return res.json(
      "izmena podataka predstave čiji je id=" +
        req.params.id +
